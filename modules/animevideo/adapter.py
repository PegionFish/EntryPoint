"""adapter.py — EntryPoint animevideo 模块适配器（W1 脚手架，实验 E6/E7/E8 载体）
...
EP_MODULE_ID = os.getenv("EP_MODULE_ID", "animevideo")
...
MODEL_TARGET_DIRS = {
    "animevideo-xsx2-pth": "animevideo-xsx2-pth",
    "animevideo-xsx4-pth": "animevideo-xsx4-pth",
}

logging.basicConfig(
    level=getattr(logging, EP_LOG_LEVEL.upper(), logging.INFO),
    format="%(asctime)s [animevideo] %(levelname)s %(message)s",
)
logger = logging.getLogger("animevideo")

app = FastAPI(title="EntryPoint animevideo adapter", version="0.1.0")
...
def run_upscale(input_path: Path, out_path: Path, params: dict) -> dict:
    ...
    # 容量护栏：长视频抽帧 PNG 可达数十 GB（真实案例：23min→42k 帧≈24GB 写爆
    # /dev/shm），估算体积超过候选盘可用量的 60% 即回落磁盘 workspace。
...
if __name__ == "__main__":
    logger.info("starting animevideo adapter on %s:%d (backend=%s)",
