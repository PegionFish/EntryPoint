...
- `desktopPages:models.*`（35 键，zh 文案以 ep-desktop/src/pages/models.rs 的 trfb 兜底为准）：filter.label=标签筛选: / filter.clear=清除 / active=激活 / vram=VRAM ≈ {{mb}} MB / run=运行 / runTip=单模型直跑：选能力 → 填参数 → 指定输入文件 / config=配置 / snapshotMissing=模块运行状态未知（请先访问仪表盘或模块页） / logs=日志 / logs.staleHint=快照数据（访问仪表盘/模块页时同步）；实时日志请打开模块详情页 / tags.editTip=编辑标签 / tags.title=编辑标签 / tags.noMeta=模型尚未下载（无 meta 文件），暂不能编辑标签 / tags.removeTip=点击移除该标签 / tags.inputHint=输入标签后回车或点添加 / tags.add=添加 / tags.hint=标签存于模型 meta，随整合包流转 / tags.saved=标签已更新 / tags.saveFailed=标签保存失败: {{detail}} / run.title=单模型直跑 / run.noManifest=模块清单不可用，无法直跑 / run.noCapability=该模块未声明任何能力（capability） / run.capability=能力 / run.browse=浏览… / run.pickFile=选择输入文件 / run.inputHint=输入文件路径 / run.submit=提交执行 / run.inputMissing=请选择存在的输入文件 / run.submitted=直跑请求已提交，进度见任务页 / config.title=变体配置（单槽位） / config.noVariants=该模块未声明模型变体 / config.apply=应用变体 / config.saved=已保存激活变体；下次启动模块生效，运行中模块需重启 / config.saveFailed=保存失败: {{detail}} / config.current=当前即为激活变体 / service.title=服务模块 / service.native=native 服务 / service.noModel=无模型模块
- `desktopApp:pipeline.*`（52 键，以 pipeline_editor.rs trfb 兜底为准）：emptyHintEdit / browse=浏览… / openTitle=打开管线 TOML / new=新建 / newTip / saveToml2=保存 / saveTip / run=执行 / runTip / refreshTasks=任务 / refreshTasksTip / palette.addTip / palette.llmTip / palette.noModules=未发现模块能力 / helpTextEdit / props=管线属性 / edgeSelected=已选中连线 / deleteEdge=删除连线 / paramLabel2=标签: / timeoutEdit=超时秒 (0=默认): / retryEdit=重试次数: / builtinNoParams / manifestMissing / modelPin=变体 pin: / followActive=跟随激活变体 / deviceBind=设备: / addArg=参数项 / argsTip / apiKeyTip / promptTip / temperature / maxTokens / deleteNode=删除节点 / vram.title=VRAM 账本 / vram.over=超预算 / vram.unknownCap=容量未知 / vram.unassigned=auto 未分配 / vram.schedulerNote / vram.noDevices / vram.suggestion / vram.overcommitOff / vram.cycle / connSelf=不能连接到自身 / connDup=该连线已存在 / connCycle=连线会形成环，已拒绝 / connType=端口类型不兼容: {{from}} → {{to}} / connOk=已连线 / pickInput=选择执行输入文件 / saveTitle=保存管线 TOML / saved=已保存: {{path}} / saveFailed=保存失败: {{detail}} / saveSerializeFailed=序列化失败: {{detail}} / execSubmitted=已提交执行，进度见任务页（门禁接线路径）/ execChannelClosed=执行通道不可用（后台循环已退出）
- `desktopApp:tasks.*`（5 键）：queuePosition=队列位置 {{pos}} / cancel=取消 / artifacts.title=输出产物 / artifacts.dirMissing=任务目录不存在（任务可能未在本机执行或产物已清理） / artifacts.openDir=打开任务目录
- `common:*`（3 键）：status.queued=排队中（与 C1 重复，取一） / label.name2=名称: / label.description2=描述:

> 旧键清理（C5 报告）：`desktopApp.pipeline.saveToml`/`saveTomlTip`/`saveNotImplemented` 已被 saveToml2/saveTip 取代，可删；`desktopApp.pipeline.execPendingWire` 与 `desktopApp.tasks.cancelPending` 接线后已无引用，可删。状态：待落盘-迟到。
