...
### 5.4 快速调用 (Quick Run) — ✅ 已实现
- 路由 `/run`，与管线编辑器同级：单模块能力 ad hoc 直跑入口，免搭管线
- **能力目录**：聚合全部已装模块声明的能力，按 category 分类筛选；
  条目显示模块名 + 能力名 + 输入→输出类型与运行状态点
- **输入区**：按能力 input_type 切换——文件型为服务器路径直填 / 浏览器上传，
  文本型为 textarea 直填（TTS 类 text→audio 能力由此可用）
- **参数表单**：由 manifest 的 params schema 自动渲染
- **提交即受理**：提交立即创建任务返回（lazy 受理），模块未运行自动拉起，
  冷启动过程作为任务节点状态可见，不阻塞提交
- **页内会话任务区**：本次会话提交的任务卡片实时展示进度与节点状态
  （WebSocket 推送），支持产物预览下载与「在任务中心查看」深链
- **引导与提示**：模型未就绪时引导跳转模块页下载；页面提示空闲自动下线
  （`[modules].idle_timeout_secs` 缺省 1800s，0=常驻，「设置 → 模块生命周期」可调）
- 模块页原有「直跑抽屉」保留不变（与本页共用参数表单 / 产物预览组件）
